...
void runIns(struct Instruction i) {	  
  switch (i.op) {
    ...
    case JMP: // src será un IMM, pues processLabels habrá modificado ya la etiqueta
		...
		break;
	case CALL: // src es un IMM (processLabels)
		if (i.src.type!=IMM)
...
int main(int argc, char *argv[]) {
  int i;
  FILE *fp = fopen(argv[1], "r");         // abro un archivo
  yyin = fp;                              // el parser leerá desde ese archivo
  yyparse();                              // se parsea el archivo 
  fclose(fp);                             // cierro el archivo
  processLabels();                        // reemplazo de etiquetas por valores inmediatos
  printf("Running the following code\n");
  for (i=0;i<count; i++)  {               // imprimimos el código a ejecutar
    printf("%d: ",i);
    printInstr(code[i]);
  }
  printf("***************\n");
  ...
  run(code);                               // ejecutamos el código
  dumpMachine();                           // exponemos el estado final
